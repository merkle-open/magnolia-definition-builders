...
public class OrAvailabilityRule implements AvailabilityRule {
	...
	public boolean isAvailable(final Collection<?> items) {
		return ruleDefinition.rules
				...
				.anyMatch(rule -> rule.isAvailable(items));
	}
	...
	public static class Definition extends ConfiguredAvailabilityRuleDefinition {
		...
		public Definition(final Set<AvailabilityRuleDefinition> rules) {
			setImplementationClass(OrAvailabilityRule.class);
			this.rules = rules;
		}

		public static class Builder extends AbstractAvailabilityRuleDefinitionBuilder<Definition, Definition.Builder> {
			...
			public Definition.Builder rule(final AvailabilityRuleDefinition rule) {
			...
			public Definition.Builder rules(final Set<AvailabilityRuleDefinition> rules) {
				this.rules = rules;
				return self();
			}
