...
public class YoutubeVideoSourceTransformer implements VideoSourceTransformer {
	...
	public Optional<VideoSource> transform(final Locale locale, final String src) {
		if (Strings.CS.startsWith(src, YoutubeTextValueConverter.PREFIX)) {
			return Optional.of(new VideoSource(Strings.CS.removeStart(src, YoutubeTextValueConverter.PREFIX), null));
		}
